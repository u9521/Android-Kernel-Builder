...
COPY . .
RUN chmod +x android_kernel_builder/docker/install-base-deps.sh \
&& android_kernel_builder/docker/install-base-deps.sh \
&& uv sync --frozen --no-dev \
&& uv run akb image-env --target "${TARGET}" \
&& install -Dm755 "android_kernel_builder/docker/entrypoint.sh" /usr/local/bin/akb-entrypoint
