...
class BuildCacheTests(unittest.TestCase):
    def test_init_build_cache_reuses_matching_outer_cache(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(build_cache, "run_command") as run_command:
                with mock.patch.object(build_cache, "_create_empty_outer_cache_image") as create_outer:
                    build_cache.init_build_cache(work_root)
    ...
    def test_init_build_cache_recreates_outer_cache_on_metadata_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            work_root = Path(temp_dir)
            ...
            with mock.patch.object(build_cache, "run_command"):
                with mock.patch.object(build_cache, "_create_empty_outer_cache_image") as create_outer:
                    build_cache.init_build_cache(work_root)

            create_outer.assert_called_once()

    def test_export_build_cache_renames_outer_cache_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            work_root = Path(temp_dir)
            outer_root = layout.docker_outerimage_root(work_root)
            outer_root.mkdir(parents=True, exist_ok=True)
            ...
            with mock.patch.object(build_cache, "run_command") as run_command:
                build_cache.export_build_cache(work_root)
    ...
    def test_finalize_build_cache_cleans_up_then_exports(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            work_root = Path(temp_dir)

            with mock.patch.object(build_cache, "cleanup_build_cache") as cleanup_build_cache:
                with mock.patch.object(build_cache, "export_build_cache") as export_build_cache:
                    build_cache.finalize_build_cache(work_root)

            cleanup_build_cache.assert_called_once_with(work_root, None)
            export_build_cache.assert_called_once_with(work_root)
    ...
    def test_create_empty_outer_cache_image_uses_sparse_10g_size(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(build_cache, "run_command") as run_command:
                build_cache._create_empty_outer_cache_image(
                    work_root,
                    build_cache._load_required_metadata(layout.docker_container_cache_metadata_file(work_root)),
                )
    ...
    def test_prepare_base_build_cache_creates_sparse_10g_image_and_mounts_cache_root(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            work_root = Path(temp_dir)
            layout.docker_datas_root(work_root).mkdir(parents=True, exist_ok=True)

            with mock.patch.object(build_cache, "run_command") as run_command:
                build_cache.prepare_base_build_cache(work_root, "sample")
            ...
            self.assertEqual(
                ...
                ["mount", "-v", "-o", "loop", str(layout.docker_container_cache_image(work_root)), str(layout.target_cache_root(work_root, "sample"))],
            )

    def test_pack_base_build_cache_shrinks_existing_image_and_writes_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            work_root = Path(temp_dir)
            layout.docker_datas_root(work_root).mkdir(parents=True, exist_ok=True)
            layout.target_cache_root(work_root, "sample").mkdir(parents=True, exist_ok=True)
            ...
            with mock.patch.object(build_cache, "run_command") as run_command:
                with mock.patch.object(build_cache, "sha256_file", return_value="sha256"):
                    build_cache.pack_base_build_cache(work_root)
